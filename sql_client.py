...
USING (VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)) AS source (
    ...
    qty, price_unit, price_subtotal, odoo_write_date
)
ON target.odoo_id = source.odoo_id
WHEN MATCHED THEN
    UPDATE SET
...
WHEN NOT MATCHED THEN
    ...
    VALUES (
        ...
        source.qty, source.price_unit, source.price_subtotal, source.odoo_write_date
    );
"""
    ...
    def upsert_sale_order_lines(self, records: list[SaleOrderLine]) -> int:
        ...
        rows = [
            (
                ...
                r.qty, r.price_unit, r.price_subtotal, r.write_date,
            )
            for r in records
        ]
        with self._connect() as conn:
